...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  ...
  it.effect("hydrates only the latest 2,000 activities per thread", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        WITH RECURSIVE activity_numbers(value) AS (
          VALUES (1)
          UNION ALL
          SELECT value + 1 FROM activity_numbers WHERE value < 2001
        )
        INSERT INTO projection_thread_activities (
          activity_id,
          thread_id,
      ...
      yield* sql`
        ...
        WHERE activity_id = 'activity-0001'
      `;

      const snapshot = yield* snapshotQuery.getSnapshot();
      const detail = yield* snapshotQuery.getThreadDetailById(
        ThreadId.make("thread-activity-window"),
      );
      const snapshotActivities = snapshot.threads[0]?.activities ?? [];

      assert.equal(detail._tag, "Some");
      assert.equal(snapshotActivities.length, 2_000);
      assert.equal(snapshotActivities[0]?.id, asEventId("activity-0002"));
      assert.equal(snapshotActivities.at(-1)?.id, asEventId("activity-2001"));
      if (detail._tag === "Some") {
        assert.deepEqual(detail.value.activities, snapshotActivities);
      }
    }),
  );

  it.effect("pages thread activities behind a stable watermark and explicit retention floor", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO projection_projects (
          ...
          'project-pages', 'Pages', '/tmp/pages',
          '{"instanceId":"codex","model":"gpt-5-codex"}', '[]',
          '2026-04-02T00:00:00.000Z', '2026-04-02T00:00:00.000Z', NULL
        )
      `;
      yield* sql`
        INSERT INTO projection_threads (
          ...
          (
            'thread-pages', 'project-pages', 'Paged',
            '{"instanceId":"codex","model":"gpt-5-codex"}', 'full-access',
            'default', NULL, NULL, NULL, NULL, 0, 0, 0,
            '2026-04-02T00:00:00.000Z', '2026-04-02T00:00:00.000Z', NULL
          ),
          (
            'thread-empty', 'project-pages', 'Empty',
            '{"instanceId":"codex","model":"gpt-5-codex"}', 'full-access',
            'default', NULL, NULL, NULL, NULL, 0, 0, 0,
            '2026-04-02T00:00:00.000Z', '2026-04-02T00:00:00.000Z', NULL
          ),
          (
            'thread-oversized', 'project-pages', 'Oversized',
            '{"instanceId":"codex","model":"gpt-5-codex"}', 'full-access',
            'default', NULL, NULL, NULL, NULL, 0, 0, 0,
            '2026-04-02T00:00:00.000Z', '2026-04-02T00:00:00.000Z', NULL
          )
      `;
      yield* sql`
        WITH activity_numbers(value) AS (VALUES (1), (2), (3), (4), (5))
        INSERT INTO projection_thread_activities (
          activity_id, thread_id, turn_id, tone, kind, summary,
          payload_json, sequence, applied_sequence, created_at
        )
        SELECT
          printf('activity-%d', value), 'thread-pages', NULL, 'tool',
          'tool.updated', printf('activity %d', value), json_object('value', value),
          CASE WHEN value = 5 THEN 500 ELSE value END, value,
          printf('2026-04-02T00:00:0%d.000Z', value)
        FROM activity_numbers
      `;
      yield* sql`
        INSERT INTO projection_thread_activities (
          ...
          'activity-unsequenced', 'thread-pages', NULL, 'info', 'runtime.note',
          'legacy activity', '{"legacy":true}', NULL, 1, '2026-04-02T00:00:00.500Z'
        )
      `;
      let sequence = 5;
      for (const projector of Object.values(ORCHESTRATION_PROJECTOR_NAMES)) {
        yield* sql`
          INSERT INTO projection_state (projector, last_applied_sequence, updated_at)
          VALUES (${projector}, ${sequence}, '2026-04-02T00:01:00.000Z')
        `;
        sequence += 1;
      }
      ...
      const pagePlan = yield* sql<{ readonly detail: string }>`
        ...
        WHERE thread_id = 'thread-pages' AND applied_sequence <= 5
        ORDER BY sequence DESC, created_at DESC, activity_id DESC
        LIMIT 3
      `;
      assert.equal(
        pagePlan.some((step) => step.detail.includes("USE TEMP B-TREE")),
        false,
      );

      const initial = yield* snapshotQuery.getThreadActivityPage(ThreadId.make("thread-pages"), {
        cursor: { kind: "initial" },
        pageSize: 2,
      });
      assert.equal(initial._tag, "Some");
      if (initial._tag !== "Some" || initial.value.kind !== "page") return;
      assert.deepEqual(
        initial.value.activities.map((activity) => activity.id),
        [asEventId("activity-4"), asEventId("activity-5")],
      );
      assert.equal(initial.value.pageInfo.asOfSequence, 5);
      assert.equal(initial.value.pageInfo.limits.pageSize, 2);
      assert.equal(initial.value.pageInfo.limits.payloadBytes, 4 * 1024 * 1024);
      assert.deepEqual(initial.value.pageInfo.retentionFloor, {
        kind: "oldest-available",
        position: {
          sequence: null,
          createdAt: "2026-04-02T00:00:00.500Z",
          activityId: asEventId("activity-unsequenced"),
        },
      });
      const nextCursor = initial.value.pageInfo.nextCursor;
      ...
      yield* sql`
        INSERT INTO projection_thread_activities (
          ...
          (
            'activity-live-reset', 'thread-pages', NULL, 'tool', 'tool.updated',
            'live reset', '{"value":6}', 1, 6, '2026-04-02T00:00:06.000Z'
          ),
          (
            'activity-live-unsequenced', 'thread-pages', NULL, 'tool', 'tool.updated',
            'live unsequenced', '{"value":7}', NULL, 6, '2026-04-02T00:00:07.000Z'
          )
      `;
      const next = yield* snapshotQuery.getThreadActivityPage(ThreadId.make("thread-pages"), {
        cursor: nextCursor,
        pageSize: 2,
      });
      assert.equal(next._tag, "Some");
      if (next._tag === "Some" && next.value.kind === "page") {
        assert.deepEqual(
          next.value.activities.map((activity) => activity.id),
          [asEventId("activity-2"), asEventId("activity-3")],
        );
        const finalCursor = next.value.pageInfo.nextCursor;
        assert.notEqual(finalCursor, null);
        if (finalCursor !== null) {
          const final = yield* snapshotQuery.getThreadActivityPage(ThreadId.make("thread-pages"), {
            cursor: finalCursor,
            pageSize: 2,
          });
          assert.equal(final._tag, "Some");
          if (final._tag === "Some" && final.value.kind === "page") {
            assert.deepEqual(
              final.value.activities.map((activity) => activity.id),
              [asEventId("activity-unsequenced"), asEventId("activity-1")],
            );
            assert.equal(final.value.pageInfo.nextCursor, null);
          }
        }
      }

      // A continuation is explicitly expired when retention advances, even
      // if its anchor row still exists. This prevents silently skipping rows
      // pruned between page requests.
      yield* sql`
        DELETE FROM projection_thread_activities
        WHERE activity_id = 'activity-unsequenced'
      `;
      const floorExpired = yield* snapshotQuery.getThreadActivityPage(
        ThreadId.make("thread-pages"),
        { cursor: nextCursor, pageSize: 2 },
      );
      assert.equal(floorExpired._tag, "Some");
      if (floorExpired._tag === "Some" && floorExpired.value.kind === "cursor-expired") {
        assert.deepEqual(floorExpired.value.retentionFloor, {
          kind: "oldest-available",
          position: {
            sequence: 1,
      ...
      const empty = yield* snapshotQuery.getThreadActivityPage(ThreadId.make("thread-empty"), {
        cursor: { kind: "initial" },
        pageSize: 10,
      });
      assert.equal(empty._tag, "Some");
      if (empty._tag === "Some" && empty.value.kind === "page") {
        assert.deepEqual(empty.value.activities, []);
        assert.deepEqual(empty.value.pageInfo, {
          asOfSequence: 5,
          nextCursor: null,
          retentionFloor: { kind: "empty" },
          limits: { pageSize: 10, payloadBytes: 4 * 1024 * 1024 },
          payloadBytes: 0,
          omittedPayloads: [],
        });
      }
      ...
      yield* sql`
        INSERT INTO projection_thread_activities (
          ...
          (
            'activity-small', 'thread-oversized', NULL, 'tool', 'tool.updated',
            'small', '{"value":1}', 1, '2026-04-02T00:00:01.000Z'
          ),
          (
            'activity-oversized', 'thread-oversized', NULL, 'tool', 'tool.updated',
            'oversized', json_object('data', replace(hex(zeroblob(1048577)), '00', '😀')),
            2, '2026-04-02T00:00:02.000Z'
          )
      `;
      const oversized = yield* snapshotQuery.getThreadActivityPage(
        ThreadId.make("thread-oversized"),
        { cursor: { kind: "initial" }, pageSize: 10 },
      );
      assert.equal(oversized._tag, "Some");
      if (oversized._tag === "Some" && oversized.value.kind === "page") {
        assert.equal(oversized.value.activities.length, 1);
        assert.equal(oversized.value.activities[0]?.id, asEventId("activity-oversized"));
        assert.equal((oversized.value.activities[0]?.payload as { kind?: string }).kind, "omitted");
        assert.equal(oversized.value.pageInfo.omittedPayloads.length, 1);
        assert.notEqual(oversized.value.pageInfo.nextCursor, null);
      }

      yield* sql`DELETE FROM projection_thread_activities WHERE activity_id = 'activity-4'`;
      const expired = yield* snapshotQuery.getThreadActivityPage(ThreadId.make("thread-pages"), {
        cursor: nextCursor,
        pageSize: 2,
      });
      assert.equal(expired._tag, "Some");
      if (expired._tag === "Some") {
        assert.equal(expired.value.kind, "cursor-expired");
        if (expired.value.kind === "cursor-expired") {
          assert.equal(expired.value.asOfSequence, 5);
        }
      }

      const detail = yield* snapshotQuery.getThreadDetailPageSnapshot(
        ThreadId.make("thread-pages"),
      );
      assert.equal(detail._tag, "Some");
      if (detail._tag === "Some") {
        assert.equal(detail.value.thread.activities.length, 4);
        assert.equal(detail.value.pageInfo?.asOfSequence, detail.value.snapshotSequence);
      }
    }),
  );
